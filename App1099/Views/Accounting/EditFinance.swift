...
struct EditFinance: View {
    ...
    init(show: Binding<Bool>, finance: Finance) {
        self._show = show
        self.name = finance.name
        self.price = finance.price
        self.benefit = finance.benefit
        self.promotion = finance.promotion
    }
    ...
    private var header: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Spacer()
                Button {
                    source.editFinance(name: name, price: price, benefit: benefit, promotion: promotion)
                    show = false
                } label: {
                    Text("Done")
                        .font(.system(size: 17, weight: .regular))
...
struct EditFinance_Preview: PreviewProvider {
    
    @State static var show = true
    
    static var previews: some View {
        EditFinance(show: $show, finance: Finance(uuid: UUID(), name: "name", price: "price", benefit: "benefit", promotion: "promotion"))
